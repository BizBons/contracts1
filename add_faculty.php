<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    { //update
      $update = "UPDATE users SET user_lname='{$lname}', user_fname='{$fname}', user_reg='{$mi}', email='{$email}' WHERE Username = $username;";
    ...
    {
      $query = "INSERT INTO users (username, user_lname, user_reg, user_fname, email, password)
    ...
    if($res != "error")
    {
      ...
      while($row = mysqli_fetch_array($result_el)){
        if($email != '')
        {
          ...
          $mail->Username = 'tukenya.studentvotingsystem@gmail.com';          // SMTP username
          $mail->Password = 'gicvoting'; // SMTP password
          $mail->SMTPSecure = 'tls';                         // Enable TLS encryption, `ssl` also accepted
          $mail->Port = 587;                                 // TCP port to connect to

          $mail->setFrom('tukenya.studentvotingsystem@gmail.com', 'TUK Student Online Voting System');
          $mail->addReplyTo('tukenya.studentvotingsystem@gmail.com', 'TUK Student Online Voting System');

          $mail->addAddress($email);   // Add a recipient
          //$mail->addCC('admin@tukenya.com');
                                    ...
                                    <tbody>
                                      <tr>
                                        <td align="center" dir="ltr" style="padding-bottom:15px;font-size:18px;line-height:32px;color:#263238">
                                          <center>
                                          <p style="font-size: 21px;margin: 0;">To vote, please go to this site</p> <br><a href="localhost/school-voting/login.php">TUK ONLINE Voting</a> <br><br>
                                          </center>
