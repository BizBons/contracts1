          ...
          foreach ($dsatz as $key => $number) {
            foreach ($number as $k => $content) {
              ...
              if($student_id !="")
              {
                ...
                $exists = mysqli_query($conn,$chk);
                if(mysqli_num_rows($exists)>0)
                { //update
                  $studentUpdate = "UPDATE student_list SET student_fname='{$data[2]}', student_lname='{$data[3]}', student_reg='{$data[4]}', student_course='{$data[5]}', student_email='{$data[8]}'
                                    WHERE student_id = $student_id;";
                  mysqli_query($conn,$studentUpdate);

                  $userUpdate = "UPDATE users SET email='{$data[8]}', user_fname='{$data[2]}', user_lname='{$data[3]}', user_reg='{$data[4]}', password='{$pass}'
                  ...
                  $Vexists = mysqli_query($conn,$chkV);
                  if(mysqli_num_rows($Vexists)<=0)
                  {
                    ...
                    $voterResultt = mysqli_query($conn,$voterInsertt);
                  ...
                  else
                  {
                    ...
                    $voterU = mysqli_query($conn,$voterUpdate);
                    if($voterU === FALSE)
                      $error = true;
                  }
                }
                ...
                { 
                  ...
                  $studentInsert = "INSERT INTO student_list (student_id, student_fname, student_lname, student_reg, student_course, student_email) VALUES ($student_id,'{$data[2]}','{$data[3]}','{$data[4]}','{$data[5]}','{$data[8]}');";
                  $studentResult = mysqli_query($conn,$studentInsert);
                  if($studentResult === FALSE)
                    $error = true;

                  //then insert to users table
                  $userInsert = "INSERT INTO users (username, email, user_fname, user_lname, user_reg, password) VALUES ($student_id,'{$data[8]}','{$data[2]}','{$data[3]}','{$data[4]}','{$pass}');";
                  $userResult = mysqli_query($conn,$userInsert);
                  if($userResult === FALSE)
                    $error = true;

                  //then insert user per election
                  $voterInsert = "INSERT INTO voter_election (username, election_id, role) VALUES ($student_id,$electionId,'voter')";
                  $voterResult = mysqli_query($conn,$voterInsert);
